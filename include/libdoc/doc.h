...
struct Pcdt {
	...
	uint32_t lcb;    //(4 bytes): An unsigned integer that 
									 //specifies the size, in bytes, of the 
				           //PlcPcd structure.
	struct PlcPcd PlcPcd; //(variable): A PlcPcd structure. 
												//As with all Plc elements, the size 
						   //that is specified by lcb MUST result in 
							 //a whole number of Pcd 
 ...
 */
struct Clx {
	struct Prc *RgPrc; //(variable): An array of Prc. If 
										 //this array is empty, the first byte of
					   //the Clx MUST be 0x02. 0x02 is invalid as 
						 //the first byte of a Prc, 
